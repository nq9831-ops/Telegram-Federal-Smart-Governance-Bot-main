...
- **Web 界面尚未开始**：模块十一目前只交付 REST API。Vue 3 前端已由产品方**拍板要做**（2026-09-19），
  但工程与**界面设计尚未开始**（界面设计须先与产品方商定）。OpenAPI 文档（springdoc 2.9.1）
  已引入、**默认关闭**（`TGG_OPENAPI_ENABLED`），启用后会暴露全部端点清单——含第三方
  TelegramBots 的 `/{botPath}`，属信息面。
