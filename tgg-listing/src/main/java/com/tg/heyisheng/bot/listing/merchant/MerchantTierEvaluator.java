 ...
 * 上踩过「低等级掩盖高风险」的坑）。
 *
 * <p><b>{@link Tier#SILVER} / {@link Tier#GOLD} 要求三个条件同时满足</b>
 * （分 AND 保证金 AND 流水）：任一维度不达标都不升档——高保证金买不来好声誉，
 * 高分也补不上保证金缺口，没有交易记录也不该拿高级别。
 * {@link Tier#BRONZE} 是<b>例外</b>：它只看「信用分 + 保证金」（不要求流水），
 * 因为在流水数据源缺失时它是唯一可达的等级（见文末说明）。
 *
