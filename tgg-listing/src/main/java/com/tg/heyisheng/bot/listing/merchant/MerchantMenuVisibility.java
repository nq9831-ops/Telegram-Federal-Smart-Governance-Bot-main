 ...
 * {@link MerchantReviewGuard}——两处若各写一份条件，迟早漂移成「菜单看得见、点了不生效」
 * 或（更糟）对无权者暴露命令存在。
 *
...
public class MerchantMenuVisibility implements MenuVisibility {
    ...
    /** 本接缝负责的命令——与调 {@code MerchantReviewGuard.isReviewer} 的 handler 一一对应。 */
    ...
    public MerchantMenuVisibility(MerchantReviewGuard guard) {
        this.guard = guard;
    }
