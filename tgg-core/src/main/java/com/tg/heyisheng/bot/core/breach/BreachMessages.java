...
public final class BreachMessages {
    ...
    /** 没有未通报事件。 */
    public static final String PENDING_EMPTY = "当前没有未通报的数据泄露事件。";

    /** 未通报列表标题。 */
    public static final String PENDING_HEAD = "尚未通报的数据泄露事件：\n";

    /** 未通报列表脚注。 */
    public static final String PENDING_FOOTER = "履行通报后：/data_breach report 编号（例：/data_breach report 1）";

    /** 登记成功的回执。 */
    public static String registered(Object id, Object deadline) {
        return "已登记泄露事件 #" + id + "，通报截止 " + deadline
                + "（72 小时内）。履行通报后用 /data_breach report " + id + " 记录。";
    }

    /** 已记录通报的回执。 */
    public static String reportRecorded(Object id) {
        return "已记录泄露事件 #" + id + " 的通报。";
    }

    /** 事件不存在、或已记录过通报。 */
    public static String reportMissingOrDone(Object id) {
        return "事件 #" + id + " 不存在，或已记录过通报（不覆盖首次通报时间——那是合规证据）。";
    }
    ...
    public static String reminder(Object id, Object scope, Object affectedCount, Object deadline) {
        return "⚠️ 数据泄露事件 #" + id + " 尚未通报："
                + scope
                + "（预计影响 " + affectedCount + " 人，通报截止 "
                + deadline + "）。请尽快履行通报义务并执行 /data_breach report "
                + id + " 记录。";
    }
}
