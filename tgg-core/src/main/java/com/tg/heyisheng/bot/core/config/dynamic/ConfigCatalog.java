...
public final class ConfigCatalog {
    ...
    private static final List<ConfigKey> KEYS = List.of(
            ...
            new ConfigKey("tgg.admin.session-ttl-hours", ConfigCategory.RUNTIME, ConfigValueType.HOURS,
                    true, true, false, "12", 1, 8760, null,
                    "后台会话有效期（小时）。改动需重启（登录策略在启动期从 AdminProperties 绑定）。"),
            ...
            new ConfigKey("tgg.escrow.notify.group-enabled", ConfigCategory.RUNTIME, ConfigValueType.BOOLEAN,
                    true, true, false, "true", null, null, null,
                    "担保交易 · 群内通知：把交易进度发到交易所在群（公开可见）。改动需重启。"),
            new ConfigKey("tgg.escrow.notify.private-enabled", ConfigCategory.RUNTIME, ConfigValueType.BOOLEAN,
                    true, true, false, "true", null, null, null,
                    "担保交易 · 私聊通知：把交易进度私聊给对方（对方可能不在那个群）。改动需重启。"),
