...
⚠️ **没有可视化界面**。原文 §12.2 要求 Vue 3 前端——产品方已拍板「要做」（2026-09-19），
但前端工程与**界面设计尚未开始**；在它交付之前，本手册的调用方式是 **HTTP + curl**（或自备 API 工具）。
...
- **无 Actuator / CSP / HSTS**：本项目未引 Actuator（容器健康检查只探 TCP 端口）；这些建议在反向代理层做。
